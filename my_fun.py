...
class TestCustomAudit(unittest.TestCase):
    ...
    def test_get_token(self):
        self.token_req = requests.get(self.token_url, verify=False,
                                      auth=(self.user, self.passw))  # credentials & host in a config file
    ...
    def test_add_custom_policy(self):
        ...
        Config = ConfigParser.ConfigParser()
        Config.read("nccm_config.ini")
        headers = {'authorization': 'Bearer ' + self.test_get_token(),
    ...
    def test_execute_policy_profile(self):
        ...
        print "-"*200
        print response.text
        with open("execute_policy_profile_response.xml", 'w')as fw:
